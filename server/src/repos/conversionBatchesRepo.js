const CONVERSION_BATCHES_COLLECTION = 'conversion_batches';

/**
 * The Image Optimizer add-on's sibling of batchesRepo — same shape, different
 * collection.
 * @param {{ db: object, FieldValue: object }} opts
 */
function createConversionBatchesRepo({ db, FieldValue }) {
  const batchesCol = db.collection(CONVERSION_BATCHES_COLLECTION);
   ...
   * succeededCount/failedCount via FieldValue.increment, and — since the
   * post-increment total must be computed from a value read inside the SAME
   * transaction — flips status to 'complete' in the same write once every job
   * has reported in. The first progress report also flips a still-'pending'
   * batch to 'processing'.
   */
...
function getConversionBatchesRepo() {
  if (!singleton) {
    const { getFirestore } = require('../config/firebase');
    const { FieldValue } = require('firebase-admin/firestore');
    singleton = createConversionBatchesRepo({ db: getFirestore(), FieldValue });
  }
  return singleton;
}

module.exports = { createConversionBatchesRepo, getConversionBatchesRepo };
